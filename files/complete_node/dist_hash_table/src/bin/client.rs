...
#[structopt(name = "Debug client", author = "Jakub Jagiello, kuba@cs.umu.se")]
struct Opt {
...
fn main() -> std::io::Result<()> {
    ...
    loop {
        ...
        match buf.trim() {
            "insert" => {
                let insert_pdu =
                    ValInsertPdu::new(ask_for_ssn(), ask_for("name"), ask_for("email"));
                udp_wrapper.send(&mut udp_socket, insert_pdu.into(), node);
            },
            "remove" => {
                let remove_pdu = ValRemovePdu::new(ask_for_ssn());
                udp_wrapper.send(&mut udp_socket, remove_pdu.into(), node);
            },
            "lookup" => {
                let lookup_pdu = ValLookupPdu::new(ask_for_ssn(), my_address.into(), my_port);
...
fn ask_for_ssn() -> String {
    let mut ssn = ask_for("ssn");
    if ssn.len() < 12 {
        println!("SSN was less than 12 chars, padding with '0'");
        ssn += "000000000000";
    }
    ssn[0..12].into()
}
